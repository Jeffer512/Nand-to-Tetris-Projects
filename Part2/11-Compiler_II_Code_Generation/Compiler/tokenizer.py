...
class Tokenizer:
    def __init__(self, file):
        ...
        self.data = [i.replace(';', ' ; ').replace('(', ' ( ').replace(')', ' ) ').replace('[', ' [ ')
                     .replace(']', ' ] ').replace('.', ' . ').replace('-', ' - ').replace('~', ' ~ ').replace('/', ' / ')
                     .replace(',', ' , ').replace('*', ' * ').split(' /  / ')[0].strip().split()
                     for i in file if i.lstrip() != '' and i.lstrip()[0] not in '/*']
    ...
    def advance(self):
        ...
        """
        if self.has_tokens():
            self.token = self.data[self.next_token]
            self.next_token += 1
            return self.token
        else:
            return self.token
